...
1.  **智能标注 Excel 数据表**
    ...
    *   **纯净数据**：数据库只存推文数据，所有主观标注都在 Excel 中，互不污染。
...
> "跟cursor有关的博主1.1-1.15主要讨论了什么，对AI coding要什么看法吗，产出md；并对他们的推文进行标注，情感倾向、是否与AI Coding有关和有没有提到AI替代人工"

#### 2. 深度投研
> "分析最近一周 AI Agent 领域的创业信号，重点关注编程工具赛道"
> "评估最近一周提到'SaaS 增长'的推文的情感倾向"

#### 3. 数据处理
> "导出 @swyx 本月的所有推文到 Excel，不需要分析 Or 标注这些推文是否涉及'融资'信息，并打分 1-5"
---
...
*   **Auto Annotation (意图驱动自动标注)**: 你只需要用自然语言描述需求（例如“标注是否包含脏话”或“打分商业价值”），Agent 就会自动理解并生成对应的标注标准（Schema），然后批量完成标注。这赋予了你进行无限维度、高度个性化统计分析的能力，彻底告别为每个新需求写代码的时代。
